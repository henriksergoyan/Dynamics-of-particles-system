...
package org.opensourcephysics.sip.project2B;
import java.awt.*;
...
public class LJParticles implements Drawable, ODE {
	...
	public double f_updated=0;
	public double g_updated=0;
  public double state[];
  public double ax[], ay[];
  ...
  public double radius; // radius of particles on screen
...
//    double rMinimumSquared = Math.pow(2.0, 1.0/3.0);
    ...
    double area = Lx *Ly;
    for (int i =0;i<N;++i) {
    	radiuses[i] = Math.sqrt(0.6*area/(N * Math.PI))*Math.random();
    }
    ...
    for(int i = 0;i<N;++i) {
      do {
        ...
        while((j<i)&&!overlap) {
          ...
          double rMinimumSquared = radiuses[i] + radiuses[j];
...
//        virialAccumulator += dx*fx+dy*fy;
  ...
  private boolean inBox(int i) {
	  double x1 = state[4*i];
      double y1 = state[4*i+2];
	if ((x1 <= Lx) && (x1 >= 0) && (y1 <= Ly) && (y1 >= 0)) {
		return true;
	}
...
private void calc_f(String type, int i, int j) {
	double x1 = state[4*i];
    double y1 = state[4*i+2];
    double x2 = state[4*j];
    double y2 = state[4*j+2];
    xc = Lx/2;
    yc = Ly/2;
	...
	if (type == "c") {
		...
		double fx =  M*Math.pow(dr,c-1) * dx;
		double fy =  M*Math.pow(dr,c-1) * dy;
		ax[i] += fx;           // use Newton's third law
        ay[i] += fy;
        totalPotentialEnergyAccumulator += dx*fx+dy*fy;
        virialAccumulator += dx*fx+dy*fy;
	}
	else{
		double dx = x2 -x1;
...
//		System.out.println(radiuses[j]);
		if (2*(radiuses[i] + radiuses[j])> dr) {
			g_updated = 1*g_updated;
	
			
		}
	
		double fx = Math.pow(dr,f_updated-1)*dx - Math.pow(dr, -g_updated-1) *dx;
		double fy = Math.pow(dr,f_updated-1)*dy - Math.pow(dr, -g_updated-1) *dy;
		ax[i] += fx;           // use Newton's third law
        ay[i] += fy;
        ax[j] -= fx;           // use Newton's third law
        ay[j] -= fy;
        totalPotentialEnergyAccumulator += dx*fx+dy*fy;
        virialAccumulator += dx*fx+dy*fy;
 	}
...
//    }
  ...
  public void getRate(double[] state, double[] rate) {
    ...
    for(int i = 0;i<N;i++) {
      ...
      rate[4*i+3] = ay[i];
//      rate[4*i+1] = 0;        // rate for velocity is acceleration
//      rate[4*i+3] = 0;
    }
    rate[4*N] = 1; // dt/dt = 1
  }
